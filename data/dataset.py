...
class SYSUDataset(Dataset):
    def __init__(self, root, mode='train', transform=None):
        ...
        if mode == 'gallery':
            img_paths = [path for path in img_paths if 'eq' not in path and int(path.split('/')[-3][-1]) in (1, 2, 4, 5)]
        elif mode == 'query':
            img_paths = [path for path in img_paths if 'eq' not in path and int(path.split('/')[-3][-1]) in (3, 6)]

        img_paths = sorted(img_paths)
        self.img_paths = img_paths
        self.cam_ids = [int(path.split('/')[-3][-1]) if 'eq' not in path and int(path.split('/')[-3][-1])!=3 else 6 for path in img_paths]

...
def eval_regdb(query_feats, query_ids, query_cam_ids, gallery_feats, gallery_ids, gallery_cam_ids, gallery_img_paths, rerank=False):
    ...
    query_feats_bn = query_feats[:, 2048 * 7:]
    gallery_feats_bbn = gallery_feats[:, :2048 * 7]
    gallery_feats_bn = gallery_feats[:, 2048 * 7:]

    if rerank:
        dist_mat = re_ranking(query_feats, gallery_feats, eval_type=False)
    else:
        # dist_mat = 1.0 * pairwise_distance(query_feats_bn[:, :2048], gallery_feats_bn[:, :2048]) \
                #  + 0.5 * sum(pairwise_distance(query_feats_bn[:, 2048*i: 2048*(i+1)], gallery_feats_bn[:, 2048*i: 2048*(i+1)]) for i in range(1, 7)) \
                #  + 0.1 * pairwise_distance(query_feats_bbn[:, :2048], gallery_feats_bbn[:, :2048])
        dist_mat = 0.1 * pairwise_distance(query_feats_bn[:, :2048], gallery_feats_bn[:, :2048]) \
                 + 0.5 * sum(pairwise_distance(query_feats_bn[:, 2048*i: 2048*(i+1)], gallery_feats_bn[:, 2048  *i: 2048*(i+1)]) for i in range(1, 7)) \
                 + 0.5 * pairwise_distance(query_feats_bbn[:, :2048], gallery_feats_bbn[:, :2048])

...
def get_trainer(dataset, model, optimizer, lr_scheduler=None, logger=None, writer=None, non_blocking=False, log_period=10,
                ...
                eval_interval=None, start_eval=None, sampler=None, scaler=None, rank=None, worldsize=None, rerank=False, p_size=8, k_size=8):
    if logger is None:
        logger = logging.getLogger()
        logger.setLevel(logging.WARN)

    # trainer
    trainer = create_train_engine(model, optimizer, rank, worldsize, scaler, non_blocking, p_size, k_size)
    
    setattr(trainer, "rerank", rerank)

    # checkpoint handler
    if rank == 0:
        handler = ModelCheckpoint(save_dir, prefix, save_interval=None, n_saved=3, create_dir=True,
                                save_as_state_dict=True, require_empty=False)
        trainer.add_event_handler(Events.EPOCH_COMPLETED(every=5), handler, {"model": model.module})
    ...
    if eval_interval > 0 and gallery_loader is not None and query_loader is not None:
        evaluator = create_eval_engine(model, rank, non_blocking)
    ...
    def epoch_started_callback(engine):
        ...
        if lr_scheduler is not None:
            ...
            if rank == 0:
                print(lr_scheduler._get_lr(epoch))
        kv_metric.reset()
        timer.reset()
    ...
    def epoch_completed_callback(engine):
        ...
        # print(model.module.query)
        # print(model.adaptive_triplet_loss.scale)

        if epoch % eval_interval == 0 and logger is not None:
            logger.info("Model saved at {}/{}_model_{}.pth".format(save_dir, prefix, epoch))

        if evaluator and ((epoch % eval_interval == 0 and epoch > start_eval) or (epoch > 100)) and rank == 0:
            torch.cuda.empty_cache()
            ...
            if r1 > engine.state.best_rank1:
                ...
                torch.save(model.module.state_dict(), "{}/model_best.pth".format(save_dir))
            ...
            if writer is not None:
                ...
                # select_id = [93, 285, 130, 108, 302, 116, 333, 31, 331, 125]
                # features = []
                # ids = []
                # for pid in select_id:
                #     ids.extend([pid for _ in range(10)])
                #     mask = q_ids == pid
                #     features.append(q_feats[mask][:10])

                #     ids.extend([pid for _ in range(10)])
                #     mask = g_ids == pid
                #     features.append(g_feats[mask][:10])
    ...
    def iteration_complete_callback(engine):
        ...
        if iter_in_epoch % log_period == 0 and iter_in_epoch > 0:
            ...
            msg = "Epoch[%d] Batch [%d] \t" % (epoch, iter_in_epoch)
